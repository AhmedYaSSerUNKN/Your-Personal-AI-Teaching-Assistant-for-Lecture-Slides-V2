...
HISTORY_WINDOW   = 3        # Q&A turns kept for multi-turn context
...
def build_rag_prompt(query: str, docs: List[Dict], history: List[Dict]) -> str:
    ...
    if history:
        ...
        for turn in history[-HISTORY_WINDOW:]:
            lines += [f"Student: {turn['question']}",
                      f"Assistant: {turn['answer'].split(chr(10)*2+'Sources:')[0]}"]
...
def parse_quiz_json(raw: str) -> List[Dict]:
    ...
    for item in data:
        ...
        if q and isinstance(opts, list) and len(opts) == 4 and isinstance(ans_idx, int) and 0 <= ans_idx <= 3:
            questions.append({"question": q, "options": [str(o).strip() for o in opts],
                               "answer_index": ans_idx, "explanation": expl})
    return questions
                ...
                if not submitted:
                    ...
                    if q.get("explanation"):
                        st.markdown(
                            ...
                            f"border-radius:0 4px 4px 0;padding:.4rem .9rem;"
                            f"font-size:.83rem;color:#1e3a6e;margin:.4rem 0 .8rem;'>"
                            f"💡 {q['explanation']}</div>",
                            unsafe_allow_html=True,
                        )
